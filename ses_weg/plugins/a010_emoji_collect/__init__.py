...
def get_image_from_url(url: str, timeout: Optional[int] = None) -> Optional[PILImage.Image]:
    """
    从 URL 下载图片
    :param url: 图片 URL
    :param timeout: 超时秒数，None 时使用 config.image_fetch_timeout
    :return: PIL Image 或 None
    """
    t = timeout if timeout is not None else getattr(config, "image_fetch_timeout", 90)
    try:
...
def save_image(path, image_byte):
    ...
    # 构造文件名，若存在则追加 (2) (3) ...
    stem, ext = timestamp, file_extension
    file_path = os.path.join(path, f"{stem}.{ext}")
    n = 2
    while os.path.exists(file_path):
        file_path = os.path.join(path, f"{stem}({n}).{ext}")
        n += 1
...
def extract_all_image_urls(reply_msg_info: dict) -> list[str]:
    """
    提取消息中所有图片的URL
...
async def reply(bot: Bot, origin_msg: UniMsg, event: MessageEvent):  # state: T_State):
    ...
    if Reply in origin_msg:
        ...
        # 获取reply消息的纯文本内容
        ori_msg_text = origin_msg.extract_plain_text().strip()
        
        # 判断是否是"add ***"格式，并解析多个关键词
        is_add_format = False
        keywords_list = []
        
        if ori_msg_text.lower().startswith("add "):
            # print('ori_msg_text = ', ori_msg_text)
            # print('ori_msg_text.lower() = ', ori_msg_text.lower())
            # print('start add')
            # 提取"add "后面的内容，按空格分割为多个关键词
            keywords_str = ori_msg_text[4:].strip()  # 去掉"add "前缀
            if keywords_str:
            ...
            # print(reply_text.lower())
            # print('start other')
            # 如果不是"add ***"格式，检查superuser权限
            # ========= 检查 superuser 权限 =========
            user_id_str = str(event.user_id)
            if user_id_str not in SUPERUSERS:
                # 非 superuser，直接返回，不处理回复
                return
            # ======================================
            
            # original msg dealing - 按空格分割为多个关键词
            ori_msg_text = origin_msg.extract_plain_text().strip()  # print('plaintext = ', ori_msg_text) # 蛾 对称
            keywords_list = [kw.strip() for kw in ori_msg_text.split() if kw.strip()]
            ...
            try:
                ...
                if forward_data and "messages" in forward_data:
                    ...
                    for keyword in keywords_list:
                        # 过滤：黑名单、单字符标点、纯数字关键词
                        if keyword not in BAN_SET and not is_single_punct(keyword) and not is_pure_number(keyword):
                            found = find_folder(keyword)
                            if found:
                                path, folder_name = found
                                found_folders.append((path, folder_name, keyword))
                    ...
                    processed_count = 0
                    # 合并转发条数多时先发确认
                    ack_threshold = getattr(config, "many_images_ack_threshold", 5)
                    if total_count >= ack_threshold:
                        try:
                            await word.send(f"正在处理合并转发 {total_count} 条消息，请稍候…")
                        except Exception:
                            pass
                    image_count = 0
                    text_count = 0
                    ...
                    for idx, msg_item in enumerate(forward_messages):
                        ...
                        # 处理每个子消息：如果有图片，保存所有图片并忽略文字
                        # 提取该子消息的所有图片URL
                        sub_msg_info = {"message": msg_segments}
                        image_urls = extract_all_image_urls(sub_msg_info)
                        
                        if image_urls:
                            # 如果该子消息有图片，保存所有图片，忽略文字
                            for url in image_urls:
                                try:
                                    PILImage_ = get_image_from_url(url)
                                    if PILImage_ is not None:
                                        # 保存到所有匹配的文件夹
                                        for path, folder_name, keyword in found_folders:
                    ...
                    # 处理完成后发送总结
                    folder_names = [fn if 'collect' not in fn else 'null' for _, fn, _ in found_folders]
                    keywords_str = " ".join(keywords_list)
                    summary_msg = f"合并转发处理完成：{keywords_str} ( {', '.join(folder_names)} ) - 图片:{image_count} 文本:{text_count}"
        ...
        # 提取所有图片URL
        image_urls = extract_all_image_urls(reply_msg_info)

        # 1) 如果有图片：保存所有图片，忽略文字
        if image_urls:
            # 多图时先发确认，避免长时间处理导致 WebSocket 超时
            ack_threshold = getattr(config, "many_images_ack_threshold", 5)
            if len(image_urls) >= ack_threshold:
                try:
                    await word.send(f"正在处理 {len(image_urls)} 张图片，请稍候…")
                except Exception as e:
                    nonebot.logger.debug(f"[emoji_collect] 发送处理中提示失败: {e}")
            ...
            for url in image_urls:
                try:
                    ...
                    nonebot.logger.error(f"处理图片失败: {e}")
            
            if saved_count == 0:
                try:
                    await word.finish("所有图片下载或解析失败")
                except Exception as e:
                    nonebot.logger.error(f"[emoji_collect] 发送失败(图片解析): {e}")
                return
            ...
            folder_names = [fn if 'collect' not in fn else 'null' for _, fn, _ in found_folders]
            keywords_str = " ".join(keywords_list)
            msg = f"已加入手牌：{keywords_str} ( {', '.join(folder_names)} ) - {saved_count}张图片"
        ...
        # 2) 如果没有图片，处理文字
            ...
            if text:
                ...
                try:
                    await word.finish("回复内容不包含可用内容（无图片且无文字）")
                except Exception as e:
                    nonebot.logger.error(f"[emoji_collect] 发送失败(无内容): {e}")
                return
            ...
            keywords_str = " ".join(keywords_list)
            await send_notice(
                f"已记录文本：{keywords_str} ( {', '.join(folder_names)} ) "
            )
